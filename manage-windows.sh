#!/usr/bin/env bash
# ============================================================================
#  Al Ghazw al-Thaqafi Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/05-al-ghazw-al-thaqafi}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alghazwalthaqafi}"
AVD_NAME="${AVD_NAME:-al_ghazw_al_thaqafi_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-ghazw-al-thaqafi-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Ghazw al-Thaqafi Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
