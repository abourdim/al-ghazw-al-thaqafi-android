/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Ghazw al-Thaqafi
...
(function() {
  ...
  // Expose NR_T globally for app.js label sync
  window.NR_T = NR_T;
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.threat-card').forEach(el => {
        if (el.style.display === 'none') return;
        const t = el.querySelector('.threat-title');
        const d = el.querySelector('.threat-desc');
        const a = el.querySelector('.action-box');
        let text = '';
        if (t) text += t.textContent + '. ';
        if (d) text += d.textContent + '. ';
        if (a) text += a.textContent;
        cards.push({ el, text: cleanText(text), type: 'threat' });
      });
    } else if (tabName === 'shield') {
      panel.querySelectorAll('.shield-card').forEach(el => {
        if (el.style.display === 'none') return;
        const t = el.querySelector('.shield-title');
        const d = el.querySelector('.shield-desc');
        const a = el.querySelector('.action-box');
        let text = '';
        if (t) text += t.textContent + '. ';
        if (d) text += d.textContent + '. ';
        if (a) text += a.textContent;
        cards.push({ el, text: cleanText(text), type: 'shield' });
      });
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Ibn Sina/gi, 'Ibne Sina')
        .replace(/Ibn Rushd/gi, 'Ibne Rouchd')
        .replace(/Al-Khwarizmi/gi, 'al Khawarizmi')
        .replace(/\bet\b/g, ', et')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Ibn Sina/gi, 'Ibben Seena')
        .replace(/Ibn Rushd/gi, 'Ibben Rushed')
        .replace(/Al-Khwarizmi/gi, 'al Kwarizmi')
  ...
  function readCurrentCard() {
    ...
    // Open expandable cards
    if ((card.type === 'threat' || card.type === 'shield') && card.el && !card.el.classList.contains('open')) {
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    // Try threats data
    const threatMatch = card.el ? card.el.id.match(/threat-(\d+)/) : null;
    if (threatMatch && typeof THREATS !== 'undefined') {
      const t = THREATS.find(x => x.id === parseInt(threatMatch[1]));
      if (t && t[duoLang]) duoText = t[duoLang].title + '. ' + t[duoLang].desc;
    }

    // Try shields data
    if (!duoText) {
      const shieldMatch = card.el ? card.el.id.match(/shield-(\d+)/) : null;
      if (shieldMatch && typeof SHIELDS !== 'undefined') {
        const s = SHIELDS.find(x => x.id === parseInt(shieldMatch[1]));
        if (s && s[duoLang]) duoText = s[duoLang].title + '. ' + s[duoLang].desc;
      }
    }
  ...
  function nextTab() {
    ...
    switchToTabNarrator(STATE.tabOrder[STATE.tabIndex]);
  }

  function switchToTabNarrator(tabName) {
  ...
  function playBook() {
    ...
    switchToTabNarrator(STATE.tabOrder[0]);
  }
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'الغزو الثقافي' : l === 'fr' ? 'L\'Invasion Culturelle' : 'Cultural Invasion',
  ...
  function toggleNarratorPanel() {
    ...
    if (!panel.classList.contains('hidden')) {
      updateLabels(); populateVoiceSelect(); syncCheckboxes();
    }
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('ghazw-narrator-speed', STATE.speed);
  }
  function onPitchChange(val) {
    ...
    localStorage.setItem('ghazw-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('ghazw-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('ghazw-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('ghazw-narrator-duo', checked); }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('ghazw-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('ghazw-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('ghazw-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('ghazw-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('ghazw-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      ...
      '.threat-card', '.shield-card', '.daily-card'
    ];
  ...
  var origRenderThreats = window.renderThreats;
  if (origRenderThreats) { window.renderThreats = function() { origRenderThreats(); setTimeout(injectSpeakButtons, 100); }; }
  var origRenderShields = window.renderShields;
  if (origRenderShields) { window.renderShields = function() { origRenderShields(); setTimeout(injectSpeakButtons, 100); }; }
